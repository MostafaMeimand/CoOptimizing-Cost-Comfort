...
Electricity_Price = pd.read_excel("C:/Users/mostafaem - adm/Desktop/ReactiveController_Commercial_9.0_min/DDRC implementation.xlsx",
...
def ReadExcel(name):
    Output = pd.read_csv("C:/Users/mostafaem - adm/Desktop/ReactiveController_Commercial_9.0_min/" + name + ".csv")
...
#%%    
Subjects = pd.read_csv("1_subjects.csv")
Subjects = Subjects.drop("Unnamed: 0", axis = 1)

for counter in range(120):
    X = X_Preparation()
    
    Agents = Subjects.loc[counter]
    ...
    X.to_csv("min_results_" + str(Agents[0]) + "_" + str(Agents[1]) + "_" + str(Agents[2]) + "_"
      + str(Agents[3]) + "_" + str(Agents[4]) + "_" + str(k_p) + ".csv")
#%%
pd.DataFrame(Subjects).to_csv("OneOccupantMin.csv")
